...
export function ProductCard({ product, memberPricing }: Props) {
  const neonPrice = useMemo(() => {
    const neonUpCharge = (product.market - product.msrp) / 2

    return product.msrp + neonUpCharge
  }, [product.msrp, product.market])
  const nonNeonPrice = product.market * 0.9
  const discountPct =
    product.market > 0
      ? Math.round(
          100 -
            ((memberPricing ? neonPrice : nonNeonPrice) / product.market) * 100,
        )
      : 0
      ...
      <Stack gap={2} style={{ flex: 1, justifyContent: "space-between" }}>
        ...
        <Stack direction="row" align="center" gap={3} wrap>
          <span style={{ fontSize: "1.25rem", fontWeight: 700 }}>
            {formatMoney(memberPricing ? neonPrice : nonNeonPrice)}
          </span>
